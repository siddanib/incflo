    ...
    for(int n = 0; n < vars.size(); n++) {
        if ( vars[n] == "velx" ) {
            ...
            if (ParallelDescriptor::MyProc() == 0) {
                ...
                MPI_Send(last_call.data(), last_call.size(), MPI_INT,
                         m_mpmd_other_root,94,MPI_COMM_WORLD);
            }
#endif
            for (int lev = 0; lev <= finest_level; ++lev) {
                if (m_nodal_vel_eta) {
            ...
            for (int lev = 0; lev <= finest_level; ++lev) {
               if (m_nodal_vel_eta) {
                   MultiFab strainrate(amrex::convert(mf[lev].boxArray(),
                                       IndexType::TheNodeType().ixType()),
                                       mf[lev].DistributionMap(),1,0);
                   compute_nodal_strainrate_at_level(lev,
                                                  &strainrate,
                                                  &m_leveldata[lev]->velocity,
                                                  Geom(lev),
                                                  m_cur_time, 0);
            ...
            for (int lev = 0; lev <= finest_level; ++lev) {
                MultiFab p_static((m_nodal_vel_eta) ?
                                    ...
                                    mf[lev].boxArray(),
                                    mf[lev].DistributionMap(),1,0);
                MultiFab strainrate(p_static.boxArray(),
                                        mf[lev].DistributionMap(),1,0);
                MultiFab inertial_num(p_static.boxArray(),
                                    mf[lev].DistributionMap(),1,0);
                if (m_nodal_vel_eta)
                {
                ...
                else
                {
                   compute_cc_hydrostatic_pressure_at_level(lev,&p_static,
                                    ...
                                    m_mu_p_surf_second,
                                    Geom(lev),0);
                   compute_strainrate_at_level(lev,&strainrate,
                                                  &m_leveldata[lev]->velocity,
                                                  Geom(lev),
                                                  m_cur_time, 0);
                }
                compute_inertial_num_at_level(lev,
                                               ...
                                               0);
                if (m_nodal_vel_eta) {
                   amrex::average_node_to_cellcenter(mf[lev],icomp,
                                                     inertial_num,0,1);
                }
                else {
                   MultiFab::Copy(mf[lev],inertial_num,0,icomp,1,0);
                }
            }
            ...
            for (int lev = 0; lev <= finest_level; ++lev) {
                MultiFab mu_I((m_nodal_vel_eta) ?
                                 ...
                                 mf[lev].boxArray(),
                               mf[lev].DistributionMap(),1,0);
                MultiFab p_static(mu_I.boxArray(),
                                  mf[lev].DistributionMap(),1,0);
                MultiFab strainrate(mu_I.boxArray(),
                                    mf[lev].DistributionMap(),1,0);
                if (m_nodal_vel_eta) {
                   compute_nodal_hydrostatic_pressure_at_level(lev,&p_static,
                                       &m_leveldata[lev]->density,
                ...
                if (m_nodal_vel_eta) {
                   ...
                   MultiFab::Copy(mf[lev],mu_I,0,icomp,1,0);
                }
            }
            ...
            for (int lev = 0; lev <= finest_level; ++lev) {
                ...
                if (m_nodal_vel_eta) {
                   ...
                   compute_cc_hydrostatic_pressure_at_level(lev,&p_static,
                                       ...
                                       m_mu_p_surf_second,
                                       Geom(lev),0);
                   MultiFab::Copy(mf[lev],p_static,0,icomp,1,0);
                }
            }
