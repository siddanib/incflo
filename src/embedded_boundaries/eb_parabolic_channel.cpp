...
class ParabolaYZ_IF : public GPUable
{
public:
    ParabolaYZ_IF(Real a, Real h)
        : m_a(a), m_h(h) {}

    [[nodiscard]] AMREX_GPU_HOST_DEVICE inline
    Real operator() (AMREX_D_DECL(Real x, Real y, Real z)) const noexcept {
        // Parabola in y-z plane; y = a*(y-h)**2
        Real parabola_surface = m_a*(y-m_h)*(y-m_h) - z;

        return parabola_surface;
    }
...
/********************************************************************************
 *                                                                              *
 * Function to create a parabolic channel                                       *
 *                                                                              *
 ********************************************************************************/
void incflo::make_eb_parabolic_channel ()
{
    // First get the box information
    // Get box information from inputs file
    ParmParse pp("box");

    if(geom[0].isAllPeriodic())
    {
        amrex::Abort("All directions cannot be periodic for parabolic channel\n");
    }
    else
    {
        /************************************************************************
        ...
        // This creates a parabola in y-z plane
        // y = a*(z-h)^2
        ParmParse pp_1("parabola");
        Real a = 0.;
        pp_1.get("aspect_ratio", a);
        a /= (0.5*(yhi-ylo));
        amrex::Print()<<"a for parabolic channel is : "<< a <<"\n";
        ParabolaYZ_IF parabola(a, 0.5*(yhi-ylo));
        auto gshop = EB2::makeShop(parabola);

        //auto parabolic_channel = makeIntersection(parabola, my_box);

        // Generate GeometryShop
        //auto gshop = EB2::makeShop(parabolic_channel);
