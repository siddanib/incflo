                                ...
                                Real time, int nghost)
{
    ...
    for (int lev = 0; lev <= finest_level; ++lev)
    {
            compute_viscosity_at_level(lev, vel_eta[lev], rho[lev],
                    vel[lev], tracer[lev], geom[lev], time, nghost_cc_nd);
    }
}
                                         ...
                                         int nghost, int comp_id)
{
    ...
    if (comp_id == 0) {
       ...
       non_newtonian_viscosity.sr_floor = m_sr_floor_second;
    }
#ifdef AMREX_USE_EB
   ...
   for (MFIter mfi(sr_mf,TilingIfNotGPU()); mfi.isValid(); ++mfi)
   {
       ...
       amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
       {
            ...
            if (conc_val >= min_conc_scnd) {
                ...
                //}
                //Real temp_val = temperature_arr(i,j,k);
                //if (!std::isfinite(temp_val) || temp_val <= Real(0.)) {
                //    amrex::Print()<< temp_val <<"\n";
                //    amrex::Abort("Bad granular temperature in second-order temperature coeff");
                //}
