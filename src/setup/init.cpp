...
void incflo::ReadParameters ()
{
    ...
    { // Prefix incflo
        ...
        if (m_KE_int > 0 && !m_two_fluid) {
            amrex::Abort("amr.KE_int > 0 requires an implemented ComputeKineticEnergy");
        }
...
void incflo::ReadIOParameters()
{
    ...
    if ( pp.query("plt_inertial_num",plt_var ) ) {
        Warning("amr.plt_* is depreciated. Please use amr.plotVariables");
        update_plotVars("inertial_num",plt_var);
    }
    if ( pp.query("plt_hydrostatic_p",plt_var ) ) {
        Warning("amr.plt_* is depreciated. Please use amr.plotVariables");
